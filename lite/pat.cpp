...
* ALLOC:	Creates Node.
********************************************/
static Pn data;					// 11/17/98 AM.  To reclaim these.
...
data.setRulelt(rulelt);		// Collect points to its corresponding rule elt.
if (dir == LEFT)
	ncoll = collect->push(&data);	// Push to the LEFT (front) of collect list.
else // if (dir == RIGHT)
	ncoll = collect->rpush(&data);	// Push at the RIGHT (end) of collect list.
